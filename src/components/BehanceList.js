...
const BehanceList = ({ items }) => {
    return (
        <section>
            <div className="grid md:grid-cols-3 lg:grid-cols-5 sm:grid-cols-2 gap-3 py-5 mt-60">
                {items.map((item) => (
                    <div key={item.featureTxt} className="category-item cursor-pointer">
                        <div className="cat-img relative overflow-hidden rounded-md">
                            <div className="bg-overlay"></div>
                            <img src={item.featureImg} alt={item.featureTxt} />
